...
- "Производство fur-tune печенья" - услуга предоставляемая внешней организацией. Так как организации необоходимо только отправить информацию о заказе - предпоалагем что сервис у них уже реализован и с нем надо только заинтегрироваться;
...
- "Система оформления и обработки заказов" и "Менеджмент качества выполнения заказов" объеденины в один сервис, так как работают с одними данными
- "Определение характеристик, основанное на результатах тестирования" - выделен в отдельный сервис, так как это единственное место в системе, у которого релизный цикл - 1 день, что бы не заблокировать недотестированной фичей в другом сервисе этот сервис есть смысл не объединять его больше ни с чем
